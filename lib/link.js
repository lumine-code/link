...
module.exports = {
  activate() {
    this.commandDisposable = lumine.commands.add("lumine-text-editor", "link:open", {
      description: "Open the URL under the cursor in a browser.",
  ...
  async openLink(event) {
    ...
    try {
      await lumine.shell.openExternal(url.href);
    } catch (error) {
      lumine.notifications.addWarning("Unable to open the link under the cursor.", {
