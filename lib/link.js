...
module.exports = {
  activate() {
    this.commandDisposable = atom.commands.add("atom-text-editor", "link:open", (event) =>
      this.openLink(event),
    );
  },
  ...
  openLink(event) {
    ...
    const editor = element?.getModel?.() ?? atom.workspace.getActiveTextEditor() ?? null;
